...
- For NEW_GOAL or REQ_UPDATE, choose the shallowest appropriate mode:
  - **Medium complexity** (touches 2-6 files, adds/modifies a focused feature, not a whole subsystem): **SUPERPOWER**. First `bring_in` target `super-advisor` to brainstorm interactively in Chat. The super-advisor will write the design doc, write the implementation plan, and spawn the `superpower` Relay when the user approves the plan. You do NOT call `spawn_relay` for SUPERPOWER.
...
2. For NEW_GOAL/REQ_UPDATE:
   - **SUPERPOWER (medium complexity, 2-6 files, focused feature):**
     1. Call `bring_in` with target `"super-advisor"` and a **detailed reason** that includes what the user wants, their exact words, and any key details they mentioned.
     2. The super-advisor will brainstorm in Chat, write a design doc and implementation plan to `.autoforge/plans/`, get user approval via questionnaires, and spawn the `superpower` Relay itself. Do not call `spawn_relay` for SUPERPOWER.
...
- For SUPERPOWER, the **only** tool call is `bring_in` target `"super-advisor"`. The super-advisor spawns the `superpower` Relay after plan approval.
